...
class SampleLoader:
    ...
    def __init__(
        ...
        samp_rate: int,
        sample_dtype: SampleBufferDataType = SampleBufferDataType.COMPLEX_FLOAT32,
    ...
    def load_samples(self, file: io.BufferedReader, sample_index: int, num_samples: int) -> Optional[np.ndarray]:
        ...
        file.seek(int(sample_index * self.sample_params.bytes_per_sample))
        num_bytes_to_read = compute_sample_array_size_bytes(
            num_samples, self.sample_params.bit_depth, self.sample_params.is_complex
        )
        ...
        if num_bytes_read < num_bytes_to_read:
            ...
            return None
        num_samples_read = int(num_bytes_read // self.sample_params.bytes_per_sample)
        if num_samples_read < num_samples:
            raise ValueError(f"Failed to read enough samples: {num_samples_read} < {num_samples}")
        
        if self.sample_dtype == SampleBufferDataType.COMPLEX_FLOAT32:
            convert_to_complex64_samples(
                memoryview(self._byte_buffer[:num_bytes_read]),
                self._sample_buffer_view[:num_samples_read * 8],
                self.sample_params.bit_depth,
                self.sample_params.is_signed,
                self.sample_params.is_integer,
                self.sample_params.is_complex,
                self.sample_params.is_i_lsb,
            )
            return np.frombuffer(self._sample_buffer[:num_samples_read * 8], dtype=np.complex64)
        elif self.sample_dtype == SampleBufferDataType.INTERLEAVED_COMPLEX_INT8:
...
# Need function that reads from stream into two adjacent buffers
# Then performs correlations on the buffers
